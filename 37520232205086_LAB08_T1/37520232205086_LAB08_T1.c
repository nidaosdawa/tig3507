...
{
    DMA->DMACHAN[DMA_CH0_CHAN_ID].DMACTL &= ~((uint32_t)0x00000002U);// DL_DMA_disableChannel(DMA, DMA_CH0_CHAN_ID);
    DMA->DMACHAN[DMA_CH0_CHAN_ID].DMASA = (uint32_t)gTriWave;//DL_DMA_setSrcAddr(DMA, DMA_CH0_CHAN_ID, (uint32_t)gTriWave);
    DMA->DMACHAN[DMA_CH0_CHAN_ID].DMADA = (uint32_t)&(DAC0->DATA0);//DL_DMA_setDestAddr    (DMA, DMA_CH0_CHAN_ID, (uint32_t)&(DAC0->DATA0));
    DMA->DMACHAN[DMA_CH0_CHAN_ID].DMASZ = WAVE_POINTS;// DL_DMA_setTransferSize(DMA, DMA_CH0_CHAN_ID, WAVE_POINTS);
    DMA->DMACHAN[DMA_CH0_CHAN_ID].DMACTL |= ((uint32_t)0x00000002U);//DL_DMA_enableChannel  (DMA, DMA_CH0_CHAN_ID);
}
